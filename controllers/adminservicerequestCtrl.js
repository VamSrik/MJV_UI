...
"use strict";
angular.module('mjv')
.controller('adminServiceRequestsCtrl', function ($scope, $timeout, $http,Upload) {
    var vm = $scope;
 alert(1);
 $scope.people = [];
        ...
        $scope.EditResquest = function(){
            ...
            $http.post('../MJV_SERVICES/services/services.php?q=editServiceRequest',$scope.data).then(function(response){
                if(response.data.type===1){
                 $http.get('../MJV_SERVICES/services/services.php?q=loadAllRequests').then(function(response){
