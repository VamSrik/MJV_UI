...
"use strict";
angular.module('mjv')
.controller('VolunteerRequestsCtrl', function ($scope, $timeout, $http,$cookies,Upload) {
...
$http.get("../MJV_SERVICES/services/services.php?q=getVolunteers&p=jj").then(function(response){
    ...
    $scope.flag= false;
        $scope.myCallback = function(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
            $('td:eq(4)', nRow).html( '<span class="status'+aData.status+'"></span>'+$scope.status[aData.status]+'');
            $('td:eq(5)', nRow).html('');
...
$scope.backNav = function(){
        ...
        $scope.someClickHandler = function(info) {
           ...
           console.log(info);
            $http.get("../MJV_SERVICES/services/services.php?q=getServiceOptions&p="+info.portal_type).then(function(response){
                                ...
                                var json = '';
                                if(info.comments!=='')
                                json = JSON.parse(info.comments);
                                $scope.listComments = json;
                                info.comments = '';
                                $scope.data= info;
                            });
   ...
   $scope.update = function(model){
        ...
        $scope.EditResquest = function(){
            ...
            //$scope.data.comments = {comment:$scope.data.comments}
